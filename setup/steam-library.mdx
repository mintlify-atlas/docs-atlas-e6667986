---
title: "Steam Library Detection and Path Configuration"
...
Each library entry in Crowbar has three columns:
...
<Steps>
  ...
  <Step title="Add secondary library paths">
    If you have additional Steam libraries on other drives, click **Add** to create a new library entry. Crowbar automatically assigns the next macro (`<library2>`, `<library3>`, and so on). Click **Browse...** in the new row and navigate to the root of that Steam library folder (e.g. `D:\SteamLibrary`).
...
- An entry can only be deleted if its **Use Count** is `0`. Remove or update any game profile paths that reference the macro before deleting the library entry.
